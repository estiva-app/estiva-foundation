...
| `@estiva-app/platform` | Peek, Ship | `estiva-app/peek`, `estiva-app/ship` | **SHA-7, 0.1.0 published, both consumers live.** The socket plumbing, refactored out of Peek's `liveTopics.ts` rather than moved — ADR 0002 §10 scores that file ❌ on constraints 1 and 2, so `git mv` was never available. Peek migrated first; Ship is the second consumer (PER-3) and the one that closed SHA-7 by receiving an event it did not publish. The row was previously attributed to SHA-2, whose PWA export now *adds to* this package. **`watchFolders` is the API the second consumer forced** — `createChannelSubscriptions` is one REQ per channel, which a workspace read cannot afford against a metered budget that refuses the 51st unpaced REQ |
